...
export default function ModuleDetails() {
    ...
    useEffect(() => {
        ...
        if (foundModule) {
            ...
            // Optional: Navigate to 404 or dashboard if not found
            // navigate('/learning-paths/bootcamp');
        }
    }, [id]);
    ...
    return (
        <main className="flex-1 overflow-y-auto relative w-full scroll-smooth">
            <div className={`absolute top-0 inset-x-0 h-96 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-${module.color}-100/40 via-purple-50/20 to-transparent dark:from-${module.color}-900/20 dark:via-background-dark dark:to-background-dark pointer-events-none z-0`}></div>
            ...
            <div className="max-w-7xl mx-auto p-6 lg:p-8 relative z-10">
                ...
                <nav aria-label="Breadcrumb" className="flex text-sm font-medium text-slate-500 dark:text-slate-400 mb-10">
                    <ol className="inline-flex items-center space-x-2">
                        ...
                        <li><span className="material-icons-round text-sm opacity-50">chevron_right</span></li>
                        <li className="text-slate-900 dark:text-white font-semibold">{module.title}</li>
                    </ol>
                </nav>
                ...
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10">
                    <div className="max-w-3xl">
                        <div className="flex items-center gap-4 mb-3">
                            <h2 className="text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-white">{module.shortTitle} Module</h2>
                            <span className={`px-3 py-1 rounded-full bg-${module.color}-100 dark:bg-${module.color}-900/30 text-${module.color}-700 dark:text-${module.color}-400 text-xs font-bold uppercase tracking-wide border border-${module.color}-200 dark:border-${module.color}-700/50 shadow-sm`}>Core</span>
                        </div>
                    ...
                    <button
                        onClick={() => navigate(`/lessons/${module.id}-intro`)}
                    ...
                    <div className="col-span-12 lg:col-span-8 space-y-8">
                        ...
                        <div className="bg-gradient-to-br from-[#FFFBEB] to-white dark:from-yellow-950/20 dark:to-surface-dark border border-yellow-100 dark:border-yellow-900/30 p-8 rounded-3xl relative overflow-hidden shadow-soft group hover:shadow-lg transition-shadow">
                            ...
                            <div className="relative z-10">
                                ...
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Why is {module.shortTitle} important?</h3>
                                ...
                                <div className="bg-surface-light dark:bg-surface-dark border border-slate-100 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-slate-900 dark:text-white">{module.shortTitle} Topics</h4>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">Core concepts required for jobs</p>
                                            </div>
                                        </div>
                                    ...
                                    {importantTopicsExpanded && (
                                        <div className="px-5 pb-5 pt-2 border-t border-slate-50 dark:border-slate-800/50">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                                                {module.curriculum && module.curriculum.map((topic, index) => (
                                                    <div key={index} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 py-1">
                                                        <span className={`material-icons-round text-sm ${topic.completed ? 'text-green-500' : 'text-slate-300'}`}>check_circle</span>
                                                        {topic.title}
                                                    </div>
                                                ))}
                                            </div>
                    ...
                    <div className="col-span-12 lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-3xl shadow-soft border border-slate-100 dark:border-slate-700">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-primary font-extrabold text-xl">{module.progress}%</span>
                            </div>
                            <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-3 mb-3 overflow-hidden">
                                <div className="bg-primary h-3 rounded-full shadow-[0_0_10px_rgba(79,70,229,0.4)]" style={{ width: `${module.progress}%` }}></div>
                            </div>
                            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 text-right">{module.completedLessons} of {module.totalLessons} sub-modules completed</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col justify-center hover:border-slate-200 transition-colors">
                                ...
                                <div className="text-2xl font-bold text-slate-900 dark:text-white">{module.estimatedTime}</div>
                            ...
                            <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col justify-center hover:border-slate-200 transition-colors">
                                ...
                                <div className="text-2xl font-bold text-slate-900 dark:text-white">{module.level}</div>
                                <div className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Requires focus</div>
                            </div>
                        </div>
                        ...
                        {module.interviewQuestions && (
                            <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-3xl shadow-soft border border-slate-100 dark:border-slate-700">
                                ...
                                <div className="space-y-1">
                                    {module.interviewQuestions.map((question, index) => (
                                        <div key={index}>
                                            ...
                                            {index < module.interviewQuestions.length - 1 && <div className="h-px bg-slate-100 dark:bg-slate-800 w-full my-2"></div>}
                                        </div>
                                    ))}
                                </div>
                            </div>
